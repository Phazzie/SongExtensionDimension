/**
 * @fileoverview Contract Tests for Real Input Validation Service
 * @purpose Ensure RealInputValidationService matches the contract exactly
 *
 * This test suite uses the same tests as MockInputValidation.test.ts
 * but with the real AI-powered implementation.
 *
 * NOTE: These tests run in fallback mode (no API key) to ensure
 * the service works even without Gemini API access.
 */
...
import { RealInputValidationService } from '../../src/services/real/RealInputValidationService'

/**
 * NOTE: This test suite is designed to work with RealInputValidationService.
 * The tests run without an API key to verify fallback behavior.
 */
describe('RealInputValidationService Contract Tests', () => {
  let service: IInputValidationService

  beforeEach(() => {
    // Initialize without API key to test fallback mode
    service = new RealInputValidationService()
  })
